  ...
  if (compact) {
    return (
      <LocalizedLink href={href} className="glass-card group grid grid-cols-[96px_1fr] items-center gap-4 p-4">
        ...
        <div>
          <h3 className="font-serif text-2xl font-semibold text-foreground transition group-hover:text-deep-orange">
            {name}
          </h3>
  ...
  return (
    <article className="glass-card overflow-hidden">
      <LocalizedLink href={href} className="grid gap-0 sm:grid-cols-[210px_1fr]">
        ...
        <div className="p-6">
          <p className="text-sm text-orange"><T k="home.productsCount" values={{ count: category.count }} /></p>
          <h2 className="mt-2 font-serif text-4xl font-semibold text-foreground">{name}</h2>
